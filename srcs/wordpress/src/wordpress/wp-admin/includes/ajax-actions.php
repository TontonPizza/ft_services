...
function wp_ajax_parse_embed() {
	...
	if ( ! empty( $no_ssl_support ) || ( is_ssl() && ( preg_match( '%<(iframe|script|embed) [^>]*srcs="http://%', $parsed ) ||
	...
	if ( strpos( $parsed, 'class="wp-embedded-content' ) ) {
		...
		$return['head']    = '<script srcs="' . $script_src . '"></script>';
		$return['sandbox'] = true;
	}
